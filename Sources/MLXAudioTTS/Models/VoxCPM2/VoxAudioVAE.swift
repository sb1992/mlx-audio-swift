...
class VoxAudioVAE: Module {
    ...
    func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        // Filter out fc_logvar (not used in inference)
        ...
        // Remap PyTorch keys to MLX module structure
        ...
        // Shape fixing: transpose conv weights if needed
        var finalWeights = [String: MLXArray]()
        for (key, w) in remapped {
            var weight = w
            if weight.ndim == 3 {
                // Conv weights may need transposition from PyTorch layout
                // MLX Conv1d: (outChannels, kH, inChannels)
                // PyTorch: (outChannels, inChannels, kH) → needs transpose(0, 2, 1) to become (outChannels, kH, inChannels)
                // But some may already be correct. We'll rely on the model update to handle shape matching.
            }
