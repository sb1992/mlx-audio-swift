...
public final class VoxCPM2Model: Module, SpeechGenerationModel, @unchecked Sendable {
    ...
    func encodeWav(_ audio: MLXArray) -> MLXArray {
        var inp = audio
        if inp.ndim == 1 {
    ...
    func tokenize(_ text: String) throws -> [Int] {
        ...
        // Match Python: tokenize without BOS
        return tokenizer.encode(text: text).map { Int($0) }
    }
    ...
    public func generate(
        ...
        let maxTokens = 2000
        let minTokens = 2
        let inferenceTimesteps = 10
        let cfgValue: Float = 2.0
        let warmupPatches = 0
        ...
        if let refAudio {
            ...
            let refFeat = encodeWav(refAudio)
        ...
        // Generation loop
        for i in 0 ..< (maxTokens + warmupPatches) {
        ...
        let audio = audioVAE.decode(allFeatsFlat)
        return audio.flattened()
    }
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        // Shape-fix remaining weights: transpose 2D weights if needed
        var result = [String: MLXArray]()
        for (key, value) in otherWeights {
            var w = value
            if w.ndim == 2 {
                // Linear weights: check if transposition is needed
                // (MLX Linear stores weights as (outFeatures, inFeatures))
                // Some PyTorch weights come as (inFeatures, outFeatures)
            }
            result[key] = w
        }
